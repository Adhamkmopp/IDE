...
function init() {
    ...
    function update_legend(color_scale, selected_variable){
        ...
        d3.select("#europe_container").append("rect")
            .attr("x", 130)
            .attr("y", 10)
            .attr("width", 300)
            .attr("height", 20)
            .style("fill", function () {
                if(selected_variable === "Social_Exclusion"){
                    d3.select(".axis_map").call(x_axis_soc);
                    d3.select("#legend_text").text("Social Exclusion value");
                    return "url(#linear-gradient_soc)";
                } else {
                    d3.select(".axis_map").call(x_axis_who);
                    d3.select("#legend_text").text("Mental wellbeing Index");
                    return "url(#linear-gradient_who)"
                }
            });
    }
    ...
    function update_map(map_data, year_selection, variable_selection) {
        ...
        var color_se7 = d3.scaleLinear()
            .domain([1.5, 2.9])
            .range(['#92B558', 'white'])
            .interpolate(d3.interpolateRgb); // interpolateHsl interpolateHcl interpolateRgb

        var color_who7 = d3.scaleLinear()
            .domain([41, 72])
            .range(['white', '#B93A32'])
            .interpolate(d3.interpolateRgb);

        var color_who11 = d3.scaleLinear()
            .domain([54, 72])
            .range(['white', '#B93A32'])
            .interpolate(d3.interpolateRgb);
    ...
    function draw_exclusion_contact(dataset){
        ...
        g.append("text")
            ...
            .attr("x", width/2)
            .attr("text-anchor", "middle")
            .text("Social Exclusion correlated to Social contact - year 2011");
